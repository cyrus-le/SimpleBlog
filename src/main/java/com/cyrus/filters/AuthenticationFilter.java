 ...
 * @author Cyrus
 */
@WebFilter(filterName = "AuthenticationFilter", urlPatterns = {"/*"}, dispatcherTypes = {DispatcherType.REQUEST, DispatcherType.FORWARD})
public class AuthenticationFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        try {
            ...
            } else if (rule != null && rule.equals("guest")) {
                if (session == null || session.getAttribute("USER") == null) {
                    chain.doFilter(request, response);
                } else {
                    httpRequest.getRequestDispatcher("searchArticle").forward(request, (HttpServletResponse) response);
                }
            } else if (rule != null && rule.equals("member")) {
                if (session == null || session.getAttribute("USER") == null) {
                    httpRequest.getRequestDispatcher("searchArticle").forward(request, (HttpServletResponse) response);
                } else {
            ...
            } else if (rule != null && rule.equals("admin")) {
                if (session == null || session.getAttribute("USER") == null) {
                    httpRequest.getRequestDispatcher("searchArticle").forward(request, (HttpServletResponse) response);
                } else {
                    UserDTO user = (UserDTO) session.getAttribute("USER");
            ...
            logger.error("Error AuthenticationFilter at: " + t.getMessage());
        }
